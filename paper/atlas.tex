...
\begin{itemize}
    ...
    \item \textbf{Curvature-Guided Hyperparameter Sweeping:} The advisor uses the top eigenvalue of a two-dimensional projected Hessian as a learning-rate heuristic. The archived one-seed HPO smoke report gives ATLAS a lower final loss than three local peer implementations, but compares different per-trial training horizons. It contains no target-loss step comparison or divergent peer trial and does not establish an optimal learning rate, a $2.5\times$ speedup, or a divergence-prevention advantage.
