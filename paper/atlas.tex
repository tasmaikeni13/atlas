...
\begin{enumerate}
    ...
    \item \textbf{Curvature Noise in Independent-Batch Finite Differences:} When separate mini-batches estimate loss at each stencil coordinate, central finite-difference curvature has variance of order $\sigma^2/(B h^4)$. The archived sharpness audit reused one batch at every stencil point, so it does not demonstrate this independent-noise scaling or the originally claimed inflation.
...
The current calibration times completed jet outputs after warming each batch shape. This model covers steady-state jet execution; compilation, calibration, reconstruction, and rendering add to total elapsed time. Its scalar-loss variance and radial Hessian-slope estimates are empirical allocation proxies, not certificates for the global risk assumptions below. Archived landscape reports predate this correction and have invalid nominal allocations.
...
\centering
\small
\caption{\textbf{Archived Comparative Benchmark.} Relative $L_2$ error and Spearman rank correlation ($\rho_s$) use a 625-coordinate reference evaluated on a fixed subset batch. Legacy jet calibration did not time completed outputs; nominal allocations are invalid. The figures do not establish full-dataset accuracy or equal-budget peer dominance.}
...
\begin{enumerate}
    ...
    \item \textbf{Archived Curvature Comparison:} ATLAS reports projected curvature error $0.0048$ and measured latency $0.52$ seconds in the Causal Transformer run; the grid reports error $0.8004$. The archived design does not isolate stochastic noise as the cause of this difference or establish equal-budget superiority.
...
\begin{itemize}
    ...
    \item \textbf{Curvature-Guided Hyperparameter Sweeping:} The advisor uses a two-dimensional projected Hessian and a guarded local quadratic model along a recent optimizer update as learning-rate heuristics. A three-seed, equal-horizon CPU smoke comparison on a learnable synthetic color task produced seed-dependent rankings: ATLAS beat random search on two seeds and Optuna TPE on one seed. The jet incurred substantially more CPU time than the peer selection methods. These smoke results do not establish an optimal learning rate, a $2.5\times$ speedup, or a divergence-prevention advantage.
